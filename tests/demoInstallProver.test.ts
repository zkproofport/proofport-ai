...
function executor(sdk='0.2.12',mcp=sdk){return vi.fn(async(args:string[],options:any)=>{
...
describe('isolated published prover installation',()=>{
 it('rejects the old action-incomplete 0.2.11 release before installing',async()=>{
  const exec=executor('0.2.11');await expect(installPublishedProver(exec)).rejects.toThrow(/stable.*0\.2\.12/);
 ...
 it('installs exact versions into a fresh private runtime without inherited credentials',async()=>{
  ...
  expect(result.sdkVersion).toBe('0.2.12');expect(result.mcpVersion).toBe('0.2.12');expect(result.exitCode).toBe(0);expect(result.durationMs).toBeGreaterThanOrEqual(0);
  ...
  expect(args).toEqual(['install','--save-exact','--registry=https://registry.npmjs.org','--ignore-scripts','--no-audit','--no-fund','--workspaces=false','@zkproofport-ai/sdk@0.2.12','@zkproofport-ai/mcp@0.2.12']);
  ...
  expect(result.npmCommand).toContain('@zkproofport-ai/mcp@0.2.12');
  const second=await installPublishedProver(executor());expect(second.runtimeDirectory).not.toBe(result.runtimeDirectory);
 });
 it.each([['0.2.10','0.2.10'],['0.2.12','0.2.10'],['0.2.10','0.2.13'],['0.2.12-beta.1','0.2.12-beta.1'],['0.2.12; echo token','0.2.12; echo token']])('rejects invalid versions before install: %s/%s',async(sdk,mcp)=>{
  const exec=executor(sdk,mcp);await expect(installPublishedProver(exec)).rejects.toThrow(/stable.*0\.2\.12/);
  expect(exec.mock.calls.some(([args])=>args[0]==='install')).toBe(false);
 });
 it.each([['0.2.12','0.2.13'],['0.2.13','0.2.12']])('allows independent stable releases: SDK %s / MCP %s',async(sdk,mcp)=>{
 ...
 it('checks installed versions against the resolved registry versions',async()=>{
  const exec=executor();exec.mockImplementation(async(args,options)=>{
   if(!created.includes(options.cwd))created.push(options.cwd);
   if(args[0]==='view')return {stdout:'"0.2.12"',stderr:''};
   await fixture(options.cwd,'0.2.13');return {stdout:'',stderr:''};
  });
