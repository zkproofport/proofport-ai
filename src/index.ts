...
import { getAgentCardHandler } from './a2a/agentCard.js';
...
import { getTeeConfig, createTeeProvider } from './tee/index.js';
...
function createApp(config: Config, agentTokenId?: bigint | null) {
  ...
  // A2A routes (Agent Card is public, no payment gate)
...
async function startServer() {
  ...
  try {
    ...
    // Register agent on ERC-8004 (if configured)
    const agentTokenId = await ensureAgentRegistered(config);
