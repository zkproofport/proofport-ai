...
export class ActionGuard {
  ...
  proofApproved=false; stakeApproved=false;
  ...
  proof(){
    ...
    if(!this.proofApproved)throw Error('The user must approve delegation and proof payment in the dApp first.');
  ...
  stake(amount:string){
    ...
    if(!this.stakeApproved)throw Error('The user must approve the verified staking transaction in the dApp first.');
